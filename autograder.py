...
parser.add_argument('--bonus_rubrics', type=int, help='Use bonus rubrics for grading', default=0)

args            = parser.parse_args()
img_folder      = args.img_folder
pred_file       = args.pred_file
num_requests    = args.num_requests
bonus_rubrics   = args.bonus_rubrics
...
grader_script           = f'{project_path}/grade_project2_p1.py'
...
for index, row in roster_df.iterrows():
    ...
    if zip_files and os.path.isfile(zip_files[0]):
        ...
        aws_pem_file_path    = f"extracted/credentials/{asuid}.pem"
        aws_pem_exist        = read_and_extract_file(logger, aws_pem_file_path)

        face_deduction_path  = "extracted/face-detection/fd_component.py"
        fd_code_exist        = read_and_extract_file(logger,face_deduction_path)

        face_recognition_path = "extracted/face-recognition/fr_lambda.py"
        fr_code_exist         = read_and_extract_file(logger, face_recognition_path)

        if not cred_values or not aws_pem_exist or not fd_code_exist or not fr_code_exist:
            sanity_pass     = False
            test_pass       = False
            test_status     = "Fail"
        ...
        if sanity_pass:
            ...
            try:
                ...
                if (len(cred_values) == 4 and is_none_or_empty(cred_values[0]) == False
                        and is_none_or_empty(cred_values[1]) == False
                        and is_none_or_empty(cred_values[2]) == False
                        and is_none_or_empty(cred_values[3]) == False):
                ...
                try:
                    ...
                    iam_ro_access_flag, ec2_ro_access_flag, sqs_full_access_flag, lambda_ro_access_flag, iot_greengrass_ro_access_flg = iam_obj.validate_policies()
                    ...
                    aws_grader          = grader_project2(logger, asuid, cred_values[0], cred_values[1])
                    iot_client_ip_adrr  = cred_values[2]
                    sqs_resp_url        = cred_values[3]

                    test_results  = aws_grader.main(asuid, num_requests, aws_pem_file_path, iot_client_ip_adrr, sqs_resp_url, img_folder, pred_file, bonus_rubrics)
